...
class HybridActor(nn.Module):
    """
    Chính sách Hybrid: chọn DC (categorical) và số GPU (categorical).
    - Input: latent h (từ encoder)
    - Output: logits_dc [B, N_dc], logits_g [B, N_g]
    """
    def __init__(self, latent_dim: int, n_dc: int, n_gpus_choices: int):
        super().__init__()
        self.n_dc = n_dc
    ...
    def forward(self, h: torch.Tensor):
        ...
        return logits_dc, logits_g

    def sample(
        self,
        h: torch.Tensor,
        mask_dc: Optional[torch.Tensor],
        mask_g: Optional[torch.Tensor],
    ) -> Tuple[Dict[str, torch.Tensor], torch.Tensor]:
        logits_dc, logits_g = self.forward(h)
        a_dc, logp_dc = sample_categorical(logits_dc, mask_dc)
        a_g,  logp_g  = sample_categorical(logits_g,  mask_g)
        logp = logp_dc + logp_g
        return {"dc": a_dc, "g": a_g}, logp
...
class QuantileCritic(nn.Module):
    ...
    Input: latent state h + action embedding (one-hot dc, one-hot g).
    ...
    def forward(self, h: torch.Tensor, a_dc: torch.Tensor, a_g: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        dc_oh = F.one_hot(a_dc, num_classes=self.n_dc).float()
        g_oh  = F.one_hot(a_g,  num_classes=self.n_g).float()
        inp = torch.cat([h, dc_oh, g_oh], dim=-1)
        q1 = self.q1(inp)
        q2 = self.q2(inp)
        return q1, q2
...
class HybridSAC(nn.Module):
    ...
    Chính sách học {dc, g}; tần số do simulator chọn.
    """
    def __init__(
        self,
        encoder: nn.Module,
        ...
        self.actor = actor
        self.critic = critic
        self.target_critic = QuantileCritic(
            getattr(encoder, 'out_dim', getattr(encoder, 'latent_dim', 256)),
        ...
        self.actor_opt = torch.optim.Adam(
            list(self.encoder.parameters()) + list(self.actor.parameters()) + [self.log_alpha],
            lr=actor_lr
        )
    ...
    def act(
        ...
        # chuẩn hoá mask
        if mask_dc is not None: mask_dc = mask_dc.to(self.device).bool()
        if mask_g  is not None: mask_g  = mask_g.to(self.device).bool()
    ...
    def _target_quantiles(
        self,
        h_next: torch.Tensor,
        mask_dc_n: Optional[torch.Tensor],
        mask_g_n: Optional[torch.Tensor]
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        logits_dc, logits_g = self.actor(h_next)
        if mask_dc_n is not None: mask_dc_n = mask_dc_n.to(self.device).bool()
        if mask_g_n  is not None: mask_g_n  = mask_g_n.to(self.device).bool()
        a_dc_n, logp_dc_n = sample_categorical(logits_dc, mask_dc_n)
        a_g_n,  logp_g_n  = sample_categorical(logits_g,  mask_g_n)
        logp_n = logp_dc_n + logp_g_n
        q1_t, q2_t = self.target_critic(h_next, a_dc_n, a_g_n)
    ...
    def update(
        ...
        """
        batch keys:
          s, a_dc, a_g, r_eff, s_next, done,
          mask_dc, mask_g, mask_dc_n(optional), mask_g_n(optional)
        ...
        with torch.no_grad():
            ...
            target = r_eff + (1 - done) * gamma * q_next
            target = target.detach()
        q1, q2 = self.critic(h, a_dc, a_g)
        ...
        h_detached = self.encoder(s)
        logits_dc, logits_g = self.actor(h_detached)
        a_dc_pi, logp_dc = sample_categorical(logits_dc, mask_dc)
        a_g_pi,  logp_g  = sample_categorical(logits_g,  mask_g)
        logp = logp_dc + logp_g
        q1_pi, q2_pi = self.critic(h_detached, a_dc_pi, a_g_pi)
