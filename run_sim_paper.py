...
from configs.paper_config import (
    ...
    build_ingresses_and_topology, build_carbon_intensity, build_router_policy
)
...
def parse_args():
    ...
    p.add_argument("--algo", type=str, default="baseline",
                   choices=[
                       "baseline",          # không điều khiển nguồn
                       "cap_uniform",       # giảm f theo bước, chọn DC có ∆P lớn nhất
                       "cap_greedy"         # khung “aggregate(tasks)” (skeleton), rơi về cap_uniform để áp dụng
                       # Có thể thêm: "joint_nf", "bandit", "carbon_cost", ...
                   ])
...
def main():
    ...
    sim = MultiIngressPaperSimulator(
        ...
        router_policy=router, coeffs_map=coeffs, carbon_intensity=carbon,
